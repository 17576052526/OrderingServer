...
namespace Common
{
    ...
    public sealed class DbHelper
    {
        ...
        /// 数据库连接字符串
        /// </summary>
        public static readonly string ConnString = System.Configuration.ConfigurationManager.ConnectionStrings["ConnString"].ToString();
        /// <summary>
        ...
        /// <summary>
        /// 执行事务，出现异常回滚且抛出异常，场合一：批量删除、新增（一条sql,多个param），场合二：多条sql，参数都在一个param中
        /// </summary>
        /// <param name="sql">可包含一条或多条sql，多条用分号或换行符隔开</param>
        /// <param name="param">param如果是集合或数组类型，则存在多少个执行多少次，否则执行一次</param>
        public static void ExecuteTran(string sql, object param = null)
        {
            IDbConnection conn = CreateConnection();
            IDbCommand comm = conn.CreateCommand();
            comm.CommandText = sql;
            try
            {
                ...
                {
                    foreach (object m in (IEnumerable<object>)param)
                    {
